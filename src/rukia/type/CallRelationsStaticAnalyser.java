 ...
 */
public class CallRelationsStaticAnalyser {
...
//        System.out.println("-----------------All call relations in class-------------------------");
//        for (CallRelation callRelation : callRelationsList) {
//            System.out.println(callRelation);
//        }
        ...
        for (FieldDeclaration field : typeDec.getFields()) {
            ...
            if (isUserDefinedType(type)) {
                for (Object o : field.fragments()) {
                    ...
                    RukiaVariable rukiaVariable = new RukiaVariable(type.toString(), variableName);
    ...
    private void analyseMethods() {
        for (MethodDeclaration methodDeclaration : typeDec.getMethods()) {
            ...
            Set<RukiaVariable> variablesList = new LinkedHashSet<>();
            ...
            for (RukiaVariable variable : variablesList) {
//                System.out.println(variable);
    ...
    private void createCallRelation(String callerClassName, String callerMethodName, String calleeClassName, String calleeMethodName) {
        ...
        RukiaCallRelation rukiaCallRelation = new RukiaCallRelation(caller, callee);
        rukiaCallRelationsList.add(rukiaCallRelation);
    }

    private void addFieldsToVariableList(Set<RukiaVariable> variablesList, Map<String, String> identifierTypeMap) {
        for (RukiaVariable rukiaVariable : fieldsList) {
            variablesList.add(rukiaVariable);
            identifierTypeMap.put(rukiaVariable.getIdentifer(), rukiaVariable.getType());
        }
    }

    private void addParametersToVariablesList(MethodDeclaration methodDeclaration, Set<RukiaVariable> variablesList, Map<String, String> identifierTypeMap) {
        ...
        for (SingleVariableDeclaration singleVariableDeclaration : parametersList) {
            ...
            if (isUserDefinedType(type)) {
                variablesList.add(new RukiaVariable(type.toString(), singleVariableDeclaration.getName().getFullyQualifiedName()));
...
//        }
    ...
    private void callRelationsFilter() {
        ...
        while (iterator.hasNext()) {
            RukiaCallRelation rukiaCallRelation = (RukiaCallRelation) iterator.next();
    ...
    public Set<RukiaCallRelation> getRukiaCallRelationsList() {
